...
> 상태: **v2.1 — 공식 출처 우선 실제 조회 / BE 생산 연동 전**
>
> 소유: AI
>
...
| 범위 | 지금 상태 | 실제/합성 구분 |
|---|---|---|
| Agent Graph 호출·Review·safe failure | 실행 가능 | typed 입력을 받은 Agent 코어는 실제 실행 |
| standalone CLI 입력 | 실행 가능 | `fixtures.py`에 고정된 비식별 합성 Case·canonical step 사용 |
| 폐업 절차 원문 | 실행 가능 | 허용된 공식 웹사이트에서 실제 fetch |
| Support Agent의 catalog·판정 | 실행 가능 | 검수됐다고 가정한 합성 catalog로 비교 |
| 기업마당 지원 공고 discovery | Graph와 별도로 실행 가능 | 실제 공식 API raw 후보이나 자격 판정에는 미사용 |
| 실제 사용자 Case 조회·저장 | 실행 불가 | BE 인증·snapshot·CAS·persistence 필요 |
...
- 기존 `BIZINFO_API_KEY`를 이용한 별도 adapter의 실제 지원 공고 후보 discovery. CLI/Graph에는 연결하지 않음
- 실제 LLM structured-output 호출과 fail-closed 결과 확인

현재 팀 proxy의 `/models`에서 확인된 모델은 `openai/gpt-4.1-mini`뿐입니다. 직접 구성요소 호출과 안전 실패는 동작하지만, 전체 Graph의 복합 의미 schema는 이 모델에서 간헐적으로 `STRUCTURED_OUTPUT_FAILED` 또는 `REVIEW_RETRY_EXHAUSTED`로 안전 종료됩니다. 2026-09-15 최종 실호출에서도 실제 공식 원문과 LLM을 사용해 `REVIEWED_PLAN / NEEDS_MORE_INFO / PASS`가 나온 실행과 `REVIEW_RETRY_EXHAUSTED`가 나온 실행을 모두 확인했습니다. 따라서 현재 상태를 “모든 자연어 요청이 안정적으로 `REVIEWED_PLAN`을 반환한다”로 보지 않으며, 생산 신뢰성을 위해 더 강한 structured-output 모델을 제공하는 endpoint와 고정 회귀 평가가 필요합니다.
...
hostname DNS 결과의 private/loopback/link-local 차단과 DNS rebinding 방어는 application allowlist만으로 완성되지 않습니다. 생산 egress/resolver에서 추가해야 합니다.

## 6. 지원사업 실제 데이터 정책

기업마당 공식 API는 현재 key로 실제 조회할 수 있습니다. 이 경로는 공고 후보와 `OFFICIAL_API` Evidence를 만들기 위한 read-only discovery입니다.

```text
기업마당 API
  → strict raw candidate
  → 원문·첨부·수정시각 확인
  → 조건·서류 구조화
...
| 변수 | 현재 상태 | 용도 |
|---|---|---|
| `BIZINFO_API_KEY` | 기존 값 동작 확인 | 기업마당 지원 공고 discovery |
| `DATA_GO_KR_SERVICE_KEY` | NTS 접근 및 휴게·일반음식점 실호출 확인 | 향후 국세청·행안부 resolver; 현재 Graph 미연결 |
| `LAW_API_OC` | 법령 resolver 도입 시 발급 필요 | 국가법령정보 공동활용 API; registry-only에는 불필요 |
| `SMES_API_TOKEN` | 선택 발급 | 중소벤처24 지원 공고 보강 |
| `FOOD_SAFETY_KOREA_API_KEY` | 선택 발급 | 식품안전나라 야간 보조 수집; 현재 runtime 미연결 |
...
| 모드 | Case | 절차 | 지원사업 | 판정 |
|---|---|---|---|---|
| unit/contract | 합성 fixture | registry/mock search + mock fetch | 합성 reviewed catalog | 결정론적 CI |
| standalone CLI | 비식별 합성 fixture | 실제 공식 registry 원문, 선택 search fallback | 합성 reviewed catalog | Agent 구조·절차 live smoke |
| support discovery | 없음 | 없음 | 실제 기업마당 raw 후보 | ingestion 입력 검증, 자격 판정 아님 |
| BE 생산 | 인증된 snapshot | 실제 공식 sources | 검수된 canonical catalog | 미구현 |
...
- 인증된 snapshot assembler와 canonical field/enum registry
- canonical procedure step ID/code/name/alias registry
- 실제 사업자번호·인허가 번호의 동의·암호화·마스킹·audit resolver
- 행안부 휴게음식점 `15154921`·일반음식점 `15154916` read-only adapter(두 서비스의 HTTP 200 실호출은 2026-09-15 재확인)
- `LAW_API_OC` 기반 법령·서식 resolver
- 기업마당 raw 공고의 원문 보존·검수·catalog version 발행 pipeline
- Evidence 저장·resolver와 cache/재검증 정책
- DNS private-address/rebinding 방어와 production egress
- PlanningCoordinator, 인증·소유권, idempotency, version/CAS
- Output/State Transition Guardrail과 atomic persistence
- Langfuse adapter와 prompt/PII redaction·비용 보존 정책
- 임의 자연어·실제 Case를 받는 외부 HTTP adapter
- 복합 semantic schema를 반복 통과할 수 있는 production LLM endpoint/model과 고정 평가셋

BE는 절차 문서 내용이나 검색결과를 작성할 필요가 없습니다. AI가 source 조회·검증·해석을 소유하고, BE는 인증된 Case와 canonical ID, 개인정보·네트워크·저장 경계를 제공합니다.
