...
| **A6** `[AI]` | **부분 완료** | 검수 재작업·호출 예산·deadline 소진 시 검수 전 초안을 버리는 `SAFE_FAILURE` | C5의 외부 실패 응답·Output Guardrail과 FE 확인 요청 연결. 현재 검수 소진 시 `requested_field_paths=[]`, `recovery_action_code=NONE`. “Review 2회 실패”와 A5의 “재계획 2회” 표현도 통일 필요 |
