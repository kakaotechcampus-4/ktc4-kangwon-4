...
- **추출의 일관성.** 사실 추출은 동작한다(아래 정정 참고). 다만 같은 입력에서 실행마다
  결과가 다르다 — `business_type`을 뽑은 실행과 뽑지 않은 실행이 있다.
- **지원금 경로의 실질.** 검수 완료가 0건이라 비교할 대상이 없다.
- **충돌·재계획 경로.** 이번 trigger는 `CASE_CREATED` 하나다.

### 정정 — 추출 품질에 대한 첫 판단은 틀렸다

처음에 "통과한 실행에서도 사실을 하나도 제안하지 않았다"고 적었다. **틀렸다.**
`source_results[0]`을 정보분석으로 읽었는데 실제 호출 순서는 절차조회가 먼저다. 정보분석
결과를 다시 확인한 값은 아래와 같고, 세 실행 모두 `attempt=1`(첫 시도 통과)이다.

| 실행 | 추출한 사실 | 부족 필드 | 질문 후보 |
|---|---|---|---|
| run-7 | `business_type="작은 카페"`, `employee_count=2` | 3 | 3 |
| run-9 | `employee_count=2` | 2 | 2 |
| run-10 | `employee_count=2` | 6 | 6 |

`business_type="작은 카페"`가 실제로 추출된 것은 이번 유령 정규값 수정이 실전에서 동작한
증거다. 전에는 이 필드가 무슨 값이든 거부됐다.

남은 문제는 품질이 아니라 **일관성**이다. 같은 입력에서 `business_type`을 뽑은 실행이 하나,
안 뽑은 실행이 둘이고 부족 필드 수도 2~6으로 흔들린다. 안 뽑는 쪽이 틀린 것은 아니다 —
빠뜨린 사실은 부족 필드로 남아 다음 질문이 된다. 다만 같은 말에 같은 결과가 나오지 않는다는
점은 기록해 둔다.
