...
- 모르는 필드를 다루는 절차 문서가 있으면 `missing_fields`와 함께 `decision_authority=LANDLORD`인 `procedure_finding`도 같이 낸다 — Supervisor가 "임대인에게 확인" ACTION을 만들 근거가 된다(2026-09-22 추가)
