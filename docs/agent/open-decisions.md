...
- **미정인 내용:** 모델을 올릴지, 스키마를 단순하게 만들지, 자체 재시도 상한을 늘릴지
- **누가 남긴 미정인가:** **검토됨.** 2026-09-19~20 실측에서 확인 (8회 중 3회)
- **쟁점:** 현재 가장 잦은 실패 원인입니다. 정보분석 Agent가 스키마에 맞는 응답을 자체 상한
  3회 안에 못 만들어 `STRUCTURED_OUTPUT_FAILED`로 끝납니다. 시간 상한보다 먼저 볼 문제입니다.
  - 모델을 올리면 성공률은 오르지만 비용과 지연이 함께 오릅니다 — 지연은 이미 60초에 닿아 있습니다
  - 스키마를 단순하게 만들면 검증이 느슨해질 수 있습니다
  - 멘토가 "비싼 모델에서 저렴한 모델로 테스트하는 방향"을 권한 것과 같은 지점입니다
- **2026-09-20 경과:** 근거 참조를 짧은 손잡이로 바꾼 뒤 정보분석 단독 10회 모두 성공.
  UUID 필사 오류는 사라졌다. 남은 실패는 모델이 올바른 참조에 더해 프롬프트 속 다른
  문자열(`INPUT_JSON`, `USER_INPUT`)을 끼워넣는 것인데, 재시도로 회복된다.
  완전히 막으려면 요청마다 허용 값을 enum으로 고정해 생성 자체를 제한해야 한다 —
  멘토 리뷰 1번의 "응답 전에 후보를 추리는" 방향이다. 아직 안 했다.
- **MVP:** 완화됨. 더 줄일지는 OD-12를 먼저 처리한 뒤 다시 본다
- **누가 정하나:** **AI 단독**
- **결정 전 안전 기본값:** 실패 시 안전 실패로 끝내고 잘못된 답을 내지 않습니다. 사용자에게는
  재시도 가능으로 보이지 않습니다(`retryable=False`) — 같은 입력으로 다시 해도 같은 모델이
  같은 어려움을 겪기 때문입니다
