...
- 정보분석은 이 값을 **일관되게 미확인으로 남깁니다.** 사용자가 "임대차 계약 기간이 아직
  남았는데"라고 말해도 유상인지 무상인지는 그 문장에 없습니다. Agent 가드레일이 추정을 막는
  것은 **의도된 동작**입니다 — 루트 `CLAUDE.md`의 "모르는 것을 아는 척하기" 금지에 해당합니다.
- 즉 **사용자가 따로 말해 주기 전까지 이 값은 채워지지 않습니다.** 첫 화면에서 "임대료를
  내십니까"를 묻지 않는 한, Case를 만드는 시점에 넣을 값이 없습니다.

> **정정 (2026-09-21 저녁).** 이 절의 앞선 판에 *"재계획 4회가 전부 이 필드에서 막혔다"*고
> 적었으나 **틀린 진단이었습니다.** 실패 원인은 이 컬럼이 아니라 Review가 "아직 모른다"는
> Blocker에 근거를 요구한 것이었고, AI 쪽 프롬프트 문제여서 고쳤습니다. 수정 후 **같은
> 입력이 `lease_status`를 모르는 상태 그대로 `REVIEWED_PLAN`까지 통과**합니다.
>
> **Agent는 이 값이 없어도 동작합니다.** 남은 문제는 저장 층 하나입니다 — DB가 NOT NULL에
> 기본값이 없어 **Case 행 자체를 만들 수 없다**는 것. 그래서 A-2는 여전히 필요하지만, 급한
> 이유가 "Agent가 멈춘다"가 아니라 "Case를 생성할 수 없다"로 바뀝니다.
...
1. **A-1, A-2** — 이게 풀려야 Case 행이 만들어지고 Hero Loop Turn 1이 성립합니다.
   (Agent 실행 자체는 이 값 없이도 됩니다 — A-2의 정정 참고)
